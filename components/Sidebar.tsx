...
import { LayoutDashboard, Video, BookOpen, Bot, LogOut, Users, MessageSquare, FileText } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, onLogout }) => {
  ...
  const navItems = [
    ...
    { id: ViewState.TEACHERS, label: 'مجلس الحكماء', icon: Users }, // Booking
    { id: ViewState.LIVE_CLASS, label: 'الفصول الحية', icon: Video },
    { id: ViewState.COURSES, label: 'المقررات', icon: BookOpen },
    { id: ViewState.RESOURCES, label: 'خزائن المعرفة', icon: FileText }, // Resources
    { id: ViewState.MESSAGES, label: 'البرديات (الرسائل)', icon: MessageSquare }, // Messaging
  ...
  return (
    <aside className="w-64 bg-nubian-stone border-l border-amber-900/30 flex flex-col h-full shadow-2xl z-20 hidden md:flex">
      ...
      <div className="p-4 border-t border-amber-900/30">
        <button 
          onClick={onLogout}
          className="flex items-center space-x-3 space-x-reverse text-stone-500 hover:text-red-400 w-full px-4 py-2 transition-colors"
        >
